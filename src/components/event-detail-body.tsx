      ...
      <div className="space-y-6 px-4 py-5">
            ...
            <div className="flex flex-wrap gap-1">
              {event.tags.map((tag, i) => (
                <span
                  ...
                  className="rounded border border-zinc-700 bg-zinc-900 px-2.5 py-1.5 font-mono text-[11px] text-zinc-300"
                  title={tag.category}
                >
                  {tag.tag}
                </span>
        ...
        {event.sensors.length > 0 && (
          <section>
            ...
            <div className="flex flex-wrap gap-1">
              {event.sensors.map((s) => (
                <span key={s} className="rounded border border-zinc-700 bg-zinc-900 px-2.5 py-1.5 font-mono text-xs text-zinc-300">{s}</span>
              ))}
            </div>
          </section>
        )}
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-1">
              {event.witnesses.map((w, i) => (
                <span key={i} className="rounded border border-zinc-700 bg-zinc-900 px-2.5 py-1.5 font-mono text-xs text-zinc-300">{w}</span>
              ))}
            </div>
          </section>
        )}
